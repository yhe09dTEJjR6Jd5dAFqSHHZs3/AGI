仅用1个Python脚本实现一个“活在电脑里的大脑”，严禁使用注释或OCR。

窗口：
出现任何问题时，在窗口上显示详细信息，避免静默失败。
...
训练模式：
①持续记录以下数据到经验池：时间戳、若干个视觉画面（256×160）、每个视觉画面在电脑屏幕上的位置、鼠标（左键按下时的位置、左键按下时的时间戳、左键释放时的位置、左键释放时的时间戳、轨迹）、操作来源（AI）
